...
def transform_preds_to_waymo_format(pred_dicts, top_k_for_eval=-1, eval_second=8):
    ...
    if eval_second == 3:
        ...
        #num_frames_in_total represents the number of frames in the ground truth trajectory
        num_frames_in_total = 61
        #num_frame_to_eval represents the number of frames to evaluate the prediction
    ...
    for scene_idx, val in enumerate(scene2preds.items()):
        ...
        for obj_idx, cur_pred in enumerate(preds_per_scene):
            ...
            object_type[scene_idx, obj_idx] = cur_pred['object_type']
            object_id[scene_idx, obj_idx] = cur_pred['object_id']

            object_type_cnt_dict[object_id_to_type[cur_pred['object_type']]] += 1
