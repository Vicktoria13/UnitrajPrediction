...
import motionnet.datasets.common_utils as common_utils
import torch
...
class BaseModel(pl.LightningModule):
    ...
    ########## Abstraction via pytortch_lightning ##########
    ...
    # sera a implementer dans les classes filles
    ...
    def compute_metrics_waymo(self, pred_dicts):
        from motionnet.models.base_model.waymo_eval import waymo_evaluation
        try:
    ...
    def log_info(self, batch, prediction, status='train'):
        ...
        #center_gt_final_valid_idx = torch.ones((inputs['center_gt_trajs'].size(0)), dtype=torch.int, device=gt_traj.device)*(self.config['future_len']-1) #inputs['center_gt_final_valid_idx']
        center_gt_final_valid_idx = torch.ones((inputs['center_gt_trajs'].size(0)), dtype=torch.int, device=gt_traj.device)*(prediction['predicted_trajectory'].size(2)-1) 
        ...
        #ade_diff = torch.norm(predicted_traj[:, :, :, :2] - gt_traj[:, :, :self.config['future_len'], :2], 2, dim=-1)
        ade_diff = torch.norm(predicted_traj[:, :, :, :2] - gt_traj[:, :, :predicted_traj.size(2), :2], 2, dim=-1)
        #ade_losses = torch.sum(ade_diff * gt_traj_mask[:,:,:self.config['future_len']], dim=-1) / torch.sum(gt_traj_mask, dim=-1)
        ade_losses = torch.sum(ade_diff * gt_traj_mask[:,:,:predicted_traj.size(2)], dim=-1) / torch.sum(gt_traj_mask, dim=-1)        
