...
class MapEncoderCNN(nn.Module):
    ...
    def __init__(self, d_k=64, dropout=0.1, c=10):
        ...
        self.c = c
        init_ = lambda m: init(m, nn.init.xavier_normal_, lambda x: nn.init.xavier_normal_(x, gain=0.02), np.sqrt(2))
        # MAP ENCODER
        fm_size = 7
...
class MapEncoderPts(nn.Module):
    ...
    def __init__(self, d_k, map_attr=3, dropout=0.1):
        ...
        self.map_feats = nn.Sequential(
            init_(nn.Linear(self.d_k, self.d_k)), nn.LeakyReLU(), nn.Dropout(self.dropout),
...
class OutputModel(nn.Module):
    ...
    def __init__(self, d_k=64):
        ...
        self.observation_model = nn.Sequential(
            init_(nn.Linear(d_k, d_k)), nn.LeakyReLU(),
            init_(nn.Linear(d_k, d_k)), nn.LeakyReLU(),
...
class PTR(BaseModel):
    def __init__(self, config, k_attr=2, map_attr=2):
        ...
        #Layer Norm between each layer of attn
        self.norm1 = nn.LayerNorm(self.d_k)
        self.norm2 = nn.LayerNorm(self.d_k)
    ...
    def temporal_attn_fn(self, agents_emb, agent_masks, layer):
        ...
        agents_emb = agents_temp_emb.view(T_obs, B, num_agents, -1)
        ################################################################
        return agents_emb
    ...
    def social_attn_fn(self, agents_emb, agent_masks, layer):
        ...
        B = agent_masks.size(0)
        N = agents_emb.size(2)
        agents_emb = agents_emb.permute(2, 1, 0, 3).reshape(N, B * T_obs, -1)
        agents_soc_emb = layer(agents_emb, src_key_padding_mask=agent_masks.view(-1, N))
        agents_soc_emb = agents_soc_emb.view(N, B, T_obs, -1).permute(2, 1, 0, 3)
        agents_emb = agents_soc_emb
        ################################################################
        return agents_emb
    ...
    def _forward(self, inputs):
        ...
        for i in range(self.L_enc):
            ...
            agents_emb = self.norm1(agents_emb)
        ################################################################
    ...
    def forward(self, batch, batch_idx):
        ...
        """
        forward for one batch

        batch : dict qui contient les données d'entrée du batch. Ses clés sont : ['batch_size', 'input_dict', 'batch_sample_count']
        
        batch['input_dict'] est un dict qui contient les données d'entrée du batch. Ses clés sont : 

        ['scenario_id', 'obj_trajs', 'obj_trajs_mask', 'track_index_to_predict', 'obj_trajs_pos', 'obj_trajs_last_pos', 'center_objects_world', 'center_objects_id', 'center_objects_type', 'map_center', 'obj_trajs_future_state', 'obj_trajs_future_mask', 'center_gt_trajs', 'center_gt_trajs_mask', 'center_gt_final_valid_idx', 'center_gt_trajs_src', 'map_polylines', 'map_polylines_mask', 'map_polylines_center', 'dataset_name', 'kalman_difficulty', 'trajectory_type']
        ...
        #Apply Mask   ##############################
        inputs_device = inputs['obj_trajs'].device
        random_numbers = torch.rand(inputs['obj_trajs_mask'][:,:,1:].size(), device= inputs_device) #Computes mask, applies after the first time step
        inputs['obj_trajs_mask'][:,:,1:][random_numbers <= 0.2] = False #Apply Mask
        #############################################
        ...
        ################# Use visualize_prediction to visualize the predictions on wandb #################
    ...
    def get_loss(self, batch, prediction):
        ...
        #[inputs['center_gt_trajs'][...,:2] ==> [61, 60, 2] represente (x y) de la trajectoire de l'agent à prédire pendant 60 frames sur 61 scénarios
        #[inputs['center_gt_trajs_mask'].unsqueeze(-1) ==> [61, 60]. Le mask sert à savoir si l'agent est présent ou non à chaque frame


        #print("center_gt_trajs_mask shape : ",inputs['center_gt_trajs_mask'].shape) 
        #print("center_gt_trajs_mask squeeze shape : ",inputs['center_gt_trajs_mask'].unsqueeze(-1).shape) # [61, 60, 1]

        #ainsi, torch.cat() permettra de concatener les 2 tensors pour avoir [61, 60, 3] ==> (x, y, mask). m
