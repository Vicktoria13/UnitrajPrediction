...
class HorizontalFlip(object):
    ...
    def __call__(self, sample_list):
        ...
        #Flippe la scene : flippe les traj + la carte
        #soit map_polylines et obj_trajs

        #on flippe les obj_trajs
        new_obj_trajs = dict_trajectory['obj_trajs'].copy()
        new_obj_trajs[:, :, 0] = -dict_trajectory['obj_trajs'][:, :, 0]
        new_obj_trajs[:, :, 1] = dict_trajectory['obj_trajs'][:, :, 1]
        ...
        new_map_center = dict_trajectory['map_center'].copy()
        new_map_center[0] = -dict_trajectory['map_center'][0]
        new_dict_trajectory['map_center'] = new_map_center

        new_obj_trajs_future_state = dict_trajectory['obj_trajs_future_state'].copy()
        new_obj_trajs_future_state[:,:,0] = - dict_trajectory['obj_trajs_future_state'][:,:,0]
        new_dict_trajectory['obj_trajs_future_state'] = new_obj_trajs_future_state
        ...
        new_trajectory_type = dict_trajectory['trajectory_type']
        '''for i in range(new_trajectory_type.shape[0]):
            if dict_trajectory['trajectory_type'][i]==2:
                new_trajectory_type[i] = 3
            if dict_trajectory['trajectory_type'][i]==3:
                new_trajectory_type[i] = 2
            if dict_trajectory['trajectory_type'][i]==4:
                new_trajectory_type[i] = 5
            if dict_trajectory['trajectory_type'][i]==5:
                new_trajectory_type[i] = 4
            if dict_trajectory['trajectory_type'][i]==6:
                new_trajectory_type[i] = 7
            if dict_trajectory['trajectory_type'][i]==7:
                new_trajectory_type[i] = 6'''
        if dict_trajectory['trajectory_type']==2:
            new_trajectory_type = 3
        if dict_trajectory['trajectory_type']==3:
            new_trajectory_type = 2
        if dict_trajectory['trajectory_type']==4:
            new_trajectory_type = 5
        if dict_trajectory['trajectory_type']==5:
            new_trajectory_type = 4
        if dict_trajectory['trajectory_type']==6:
            new_trajectory_type = 7
        if dict_trajectory['trajectory_type']==7:
            new_trajectory_type = 6
        new_dict_trajectory['trajectory_type'] = new_trajectory_type
...
class Lane_Random_Occlusion(object):
    ...
    def __call__(self, sample_list):
        ...
        print("performing Lane_Random_Occlusion")
        dict_trajectory = sample_list[0]
        new_dict_trajectory = dict_trajectory.copy()
...
class Apply_noise_on_whole_trajectory(object):
    """
    given a dict = 1 scene, apply a noise trajectory on the ego agent
    """
    ...
    def __call__(self, sample_list):
        ...
        it represents 1 scene !!
        The goal of this function is, from one trajectory, create a new one
               
       """

        dict_trajectory = sample_list[0]
        new_dict_trajectory = dict_trajectory.copy()
        index_ego = new_dict_trajectory['track_index_to_predict']
        new_obj_trajs = new_dict_trajectory['obj_trajs']

        noise = np.random.randn(2)*self.std_noise #Computes starting noise

        tensor_input = dict_trajectory['obj_trajs'][index_ego, :, :2]

        #Computes change in orientation of the curves
        speed = np.diff(tensor_input,axis=0)
        orientation = np.arctan2(speed[:,1], speed[:,0])
        derive_orientation = np.diff(orientation,axis=-1)

        #Computes the rotation matrix due to the variation in orientation
        rotation_matrix = np.zeros((2,2,derive_orientation.shape[0]))
        for j in range(derive_orientation.shape[0]):
            rotation_matrix[:,:,j] = np.array([[np.cos(derive_orientation[j]), -np.sin(derive_orientation[j])], [np.sin(derive_orientation[j]), np.cos(derive_orientation[j])]])
        
        #Computes adequate noise of every time step
        new_noise = np.zeros((tensor_input.shape[0],2))
        new_noise[0,:] = noise
        new_noise[1,:] = noise
        for j in range(derive_orientation.shape[0]):
            new_noise[j+2,:] = np.matmul(rotation_matrix[:,:,j],new_noise[j+1,:])

        #Applies noise on every time step, reduces the magnitude of the noise through the steps
        for j in range(tensor_input.shape[0]):
            new_obj_trajs[index_ego, :, 0:2] += (new_noise[j,:]*(1-(j+1)/tensor_input.shape[0]))
        
        new_dict_trajectory['obj_trajs'] = new_obj_trajs
        return [new_dict_trajectory]

class Apply_noise_on_every_agent_pos(object):
    """
    given a dict = 1 scene, apply noise on the position of every agent
    """
    
    def __init__(self, std_noise = 0.05):
        self.std_noise = std_noise
    ...
    def __call__(self, sample_list):
        ...
        """
        Create a noise applied on each agent trajectory
        dict_trajectory: dictionary containing
       ...
       """
        ...
        #inputs_device = new_dict_trajectory.device

        if index_ego!=0:
            new_obj_trajs[index_ego+1:, :, :2] += np.random.randn(*new_obj_trajs[:index_ego, :, :2].shape) * self.std_noise
        new_obj_trajs[index_ego+1:, :, :2] += np.random.randn(*new_obj_trajs[index_ego+1:, :, :2].shape) * self.std_noise
...
def train(cfg):
    ...
    train_set = build_dataset(cfg, transform=[])#Apply_noise_on_start_pos(),HorizontalFlip(),Apply_noise_on_every_agent_pos(),Apply_noise_on_whole_trajectory()])
    ...
    trainer = pl.Trainer(
        ...
        profiler="simple",
        callbacks=call_backs, strategy='ddp_find_unused_parameters_true'
    )
    ...
    ######### LAUNCH TRAINING ########
        #when .fit is used, it means that the model is trained
