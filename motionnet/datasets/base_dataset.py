...
class BaseDataset(Dataset):
    ...
    def __init__(self, config=None,is_validation=False, transform=[]):
        ...
        """
        transform = None si aucune transformation n'est appliquée
        transform = [transform1, transform2, ...] si plusieurs transformations sont appliquées
        """
        ...
        self.config = config
        #data_loaded is a list of file paths
        self.data_loaded = []

        #data_loaded_memory is a list of data
        self.data_loaded_memory = []
        self.data_chunk_size = 8
        self.load_data()


        #on ajoute a la suite, les tuples (el,'augm1') pour chaque element de data_loaded
        
 
        # [transform1, transform2, ...]
        self.transform = transform

        ### supposons qu'il y ait nb elements dans self.data_loaded.
        ### Si on choisit l'option TRANSFORMATION, self.data_loaded contiendra nb*nb_type_transformations elements
        ### Sinon, contiendra nb elements

        self.nb_type_transformations = len(self.transform)
        normal = [(el,-1) for el in self.data_loaded]

        print("IF NO AUGMENTATION, WE HAVE {} ELEMENTS".format(len(normal)))

        if self.nb_type_transformations > 0:
            print('\n ====================================== Training Data will be transformed using {} transformations ======================================'.format(self.nb_type_transformations))
            """
            normal = [(el1,-1), (el2,-1), (el3,-1), ...]
            augmented = [(el1,0), (el2,0), (el3,0), ... (el1,1), (el2,1), (el3,1), ... (el1,2), (el2,2), (el3,2), ...]
            0 : transformation self.transform[0]
            1 : transformation self.transform[1]
            ...
            """
            for i in range(self.nb_type_transformations):
                augmented = [(el, i) for el in self.data_loaded]
                normal += augmented

            self.data_loaded_aug = normal
            np.random.shuffle(self.data_loaded_aug) #add shuffle
            print("IF AUGMENTATION, WE HAVE {} ELEMENTS".format(len(self.data_loaded_aug)))

        # si tableau vide, on ne fait pas de transformation
        else:
            self.data_loaded_aug = normal
    ...
    def process(self, internal_format):
        ...
        ret_dict = {
            ...
            'obj_trajs': obj_trajs_data, #representing the past trajectory of the agents
            'obj_trajs_mask': obj_trajs_mask, #masking the agents that are not present
    ...
    def postprocess(self, output):
        ...
        """
        Permet de rajouter des informations sur les données
        """
    ...
    def __getitem__(self, idx):
        ## si on a stocké les données en mémoire, on les récupère directement
        if self.config['store_data_in_memory']:
            return self.data_loaded_memory[idx]
        
        #sinon, on les charge depuis le disque
        else:
    ...
    #c'est une function qui sera appelée dès qu'on fera train_set[i]
    def __getitem__(self, idx):
        """
        on opere sur self.data_loaded_og qui est une liste de tuples
        data_loaded_aug est un tableau de tuples (el, -1) ou (el,0) ou (el,1) ou (el,2) ...

        pickle.load permet de charger les données : ainsi data est un dictionnaire
        """

        if self.config['store_data_in_memory']:
            return self.data_loaded_aug[idx]
        
        #sinon, on les charge depuis le disque et applique les transformations
        else:
            data_path, augm_type = self.data_loaded_aug[idx]
          
            with open(data_path, 'rb') as f:
                data = pickle.load(f)

            #Si aucune transformation n'est appliquée
            if augm_type == -1:
                return data
            
            #Si une transformation est appliquée
            else:
